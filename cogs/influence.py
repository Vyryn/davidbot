...
class Influence(commands.Cog):
    ...
    async def infranks(self, ctx):
        ...
        ranks_dist = {'Distinguished Associate': ranks_counter['Distinguished Associate'],
    ...
    async def top(self, ctx, amount_to_return=5):
        ...
        presentation = f'The {amount_to_return} people with the most **current** influence are:\n'
        i = 0
        # Top Total Influence
        for user in sorted(total_inf_dict.items(), key=lambda item: item[1], reverse=True):
            i += 1
        ...
        for user in sorted(lifetime_inf_dict.items(), key=lambda item: item[1], reverse=True):
            i += 1
            if i > amount_to_return:
                break
            presentation += f'{i}) {user[0]}\n'
    ...
    async def checkinf(self, ctx):
        ...
        people = {}
        for person in query:
            people[person['name']] = (person['name'], person['totalInfluence'], person['lifetimeInfluence'])
        # print(corporateer_dict)

        name = ctx.author.nickname
        if name is None:
            name = ctx.author.name
        p_o_e = person.get(name, -1)
        if p_o_e == -1:
        ...
        await ctx.send(f'{name}, your liftime influence is {people[p_o_e][2]} and your current influence balance '
                       f'is {people[p_o_e][1]}')
